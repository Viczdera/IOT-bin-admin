...
function EditOptions(props: {
  ...
  const submitForm = () => {
    let check = props.formValues.some((e: any) =>
      e.name == optionName && props.formValues[props.index]?.name != optionName
        ? true
        : false
    );
    console.log(check);
  ...
  useEffect(() => {
    props.isOpen && setOptionsData(props.formValues[props.index]);
    props.isOpen && setOptionName(props.formValues[props.index]?.name);
  ...
  useEffect(() => {
    ...
    newFormValues[props.index] = optionsData;
    //push
  ...
  return (
    ...
    >
      ...
      <DrawerContent pt="80px">
        ...
        <DrawerHeader>Edit Option</DrawerHeader>
